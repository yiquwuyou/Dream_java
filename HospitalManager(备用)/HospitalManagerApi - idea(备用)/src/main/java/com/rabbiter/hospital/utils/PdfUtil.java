//该Java类是一个工具类，用于导出PDF文件。它实现了一个静态方法ExportPdf，该方法接收一个HttpServletRequest对象、一个HttpServletResponse对象和一个Orders对象作为参数，用于生成PDF文件。
//
//        具体的实现思路是：
//
//        首先设置响应头部，指定内容类型为application/pdf。
//        创建一个Document对象，设置页面大小为A4。
//        创建两个段落，一个用于标题，一个用于打印时间，并设置字体样式和居中显示。
//        创建一个PdfWriter对象，将文档和输出流关联起来。
//        打开文档，设置文档的元数据（如标题、作者、关键字等）。
//        将段落添加到文档中。
//        创建一个表格，设置表格的样式和列数。
//        将表格添加到文档中。
//        创建病情表格，设置表格的样式和单元格内容。
//        将病情表格添加到文档中。
//        设置底部版权说明。
//        关闭文档。
