...
public class Doctor implements Serializable {
    ...
    @Excel(name = "性别")
    private String dGender;     // 性别
    @JsonProperty("dCard")
    @Excel(name = "身份证号码")
    private String dCard;       // 身份证号
    @JsonProperty("dEmail")
    @Excel(name = "邮箱")
    ...
    @JsonProperty("dPost")
    @Excel(name = "职位")
    private String dPost;        // 职位
    @JsonProperty("dIntroduction")
    @Excel(name = "简介")
    private String dIntroduction;    // 医生简介
    @JsonProperty("dSection")
    @Excel(name = "科室")
    private String dSection;        // 科室
    @JsonProperty("dState")
    @Excel(name = "是否在职", replace = {"在职_1","离职_0"})
    private Integer dState;       //是否在职   必须为integer类型，为int类型的话更新时会自动更新为0
    @JsonProperty("dPrice")
    @Excel(name = "挂号价格")
    private Double dPrice;        // 挂号价格
    @JsonProperty("dPeople")
    @Excel(name = "评价人数")
    private Integer dPeople;        //评价人数     必须为integer类型，为int类型的话更新时会自动更新为0
    @JsonProperty("dStar")
    @Excel(name = "总分")
    private Double dStar;          // 总分 必须为integer类型，为int类型的话更新时会自动更新为0
    @JsonProperty("dAvgStar")
    @Excel(name = "平均分")
    private Double dAvgStar;       // 平均分  必须为integer类型，为int类型的话更新时会自动更新为0

    /**
     * 是否已排班，排班id
     */
    @TableField(exist = false)       // 表示 arrangeId 实际上在数据库中不存在
