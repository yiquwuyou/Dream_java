...
//这段代码是一个 Spring Boot 控制器，用于处理挂号相关的请求。以下是它的主要功能：
//
//        updateOrder 方法：
//
//        接收一个 JSON 格式的 Orders 对象，用于更新挂号信息。
//        调用 orderService.updateOrder(orders) 方法，更新挂号信息。
//        updatePrice 方法：
//
//        接收挂号单的 ID oId。
//        调用 orderService.updatePrice(oId) 方法，设置缴费状态。
//        findOrderFinish 方法：
//
//        接收分页参数 pageNumber、size、查询关键字 query 和医生的 ID dId。
//        调用 orderService.findOrderFinish(pageNumber, size, query, dId) 方法，查找医生已完成的挂号单。
//        findOrderByDid 方法：
//
//        接收分页参数 pageNumber、size、查询关键字 query 和医生的 ID dId。
//        调用 orderService.findOrderByDid(pageNumber, size, query, dId) 方法，根据医生的 ID 查询挂号信息。
//        orderPeople 方法：
//
//        调用 TodayUtil.getTodayYmd() 获取当前日期。
//        调用 orderService.orderPeople(oStart) 统计今天挂号人数。
//        orderPeopleByDid 方法：
//
//        接收医生的 ID dId。
//        调用 TodayUtil.getTodayYmd() 获取当前日期。
//        调用 orderService.orderPeopleByDid(oStart, dId) 统计今天某个医生的挂号人数。
//        orderSeven 方法：
//
//        获取过去 20 天的挂号人数，返回一个整数列表。
//        orderGender 方法：
//
//        统计挂号男女人数。
//        updateOrderByAdd 方法：
//
//        接收一个 JSON 格式的 Orders 对象，用于增加诊断及医生意见。
//        调用 orderService.updateOrderByAdd(order) 方法，增加诊断及医生意见。
//        findTotalPrice 方法：
//
//        接收挂号单的 ID oId。
//        调用 orderService.findTotalPrice(oId) 方法，判断诊断之后再次购买药物是否已缴费。
//        findOrderTime 方法：
//
//        接收排班的 ID arId。
//        调用 orderService.findOrderTime(arId) 方法，请求挂号时间段。
//        orderSection 方法：
//
//        统计过去 20 天挂号科室人数。
//        这段代码实现了挂号相关的功能，包括医生的挂号管理、患者的挂号查询、统计挂号人数等操作，并根据不同的请求返回不同的响应数据，用于与前端进行交互
@RestController
