...
import com.example.demo.constant.AutoFillConstant;
...
public class AutoFillAspect {
     ...
     * 切入点
     */
    // execution(* com.example.demo.mapper.*.*(..)) 是一个切入点表达式，用于定义切入点，即确定在哪些方法上应用切面
    // execution() 是切入点指示符的关键字，用于定义方法的执行。
    // * 表示匹配任意返回类型，可以是任何类型。
    // com.sky.mapper 是包名，表示要匹配的目标方法所在的包。
    // *.* 表示任意类和方法名。
    // (..) 表示匹配任意参数，可以是任意个数和类型的参数
    @Pointcut("execution(* com.example.demo.mapper.*.*(..)) && @annotation(com.example.demo.annotation.AutoFill)")
    ...
    public void autoFill(JoinPoint joinPoint) {
        ...
        // 准备赋值的数据 - 当前用户(此处先注释，暂时不太用得到，但后续会很有用)
//        Long currentId = Long.valueOf(UserJwt.getUserId());
...
//                    setUpdateUser.invoke(entity,currentId);
            ...
            try {
                Method setUpdateTime = entity.getClass().getDeclaredMethod(AutoFillConstant.SET_UPDATE_TIME, LocalDateTime.class);
//                Method setUpdateUser = entity.getClass().getDeclaredMethod(AutoFillConstant.SET_UPDATE_USER, Long.class);
                log.info("setUpdateTime: {}", setUpdateTime + "当前时间为：" + now);
                // 通过反射为对象属性赋值
                setUpdateTime.invoke(entity, now);
//                setUpdateUser.invoke(entity, currentId);
            } catch (Exception e) {
