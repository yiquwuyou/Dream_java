...
export default {
    ...
    data() {
        return {
            //size（每页显示的条目数量）、query（查询关键词）、bedData（床位数据数组）、
            // total（总记录数）、addFormVisible（控制新增床位表单的显示与隐藏）、addForm（新增床位的表单数据）、
            // rules（表单验证规则）
            pageNumber: 1,
            size: 8,
            query: "",
    ...
    methods: {
        ...
        emptyBed(id) {
            //用于处理床位的清空、删除、新增等操作。
            // 这些方法会通过发起HTTP请求与后端进行交互，并在操作完成后更新页面数据。
            request
                .get("bed/emptyBed", {//请求地址
                    params: {
                        bId: id,//床位标识
                    },
                })
                .then((res) => {
                // 当请求成功后执行的回调函数。
                // 更新页面数据，调用 this.requestBeds() 方法重新获取床位数据。
                    this.requestBeds();
                    // 打印服务器返回的响应信息到控制台。
                    console.log(res);
                });
